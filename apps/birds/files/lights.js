...
async function setup() {
  ...
  console.log(
    "Solution: [" +
      math.transpose(
        math
          .lusolve(MatrixGame, new Array(LenghtArray).fill(1))
          .map((i) => ((i % 2) + 2) % 2)
      ) +
      "]"
  );
  ...
  for (i = 0; i < LenghtArray; i++) {
    document
      .getElementById("light-wrapper")
      .appendChild(
        await CreateImage(i, "lamp" + VettPicChosen[i], "L", "imglamps")
      );
    document
      .getElementById("button-wrapper")
      .appendChild(await CreateImage(i, "", "B", "imgbuttons", ChangeStatus));
  }
...
function ChangeStatus() {
  ...
  for (var i = 0; i < LenghtArray; i++) {
    if (MatrixGame[i][n] == 1) {
      ...
      document.getElementById("L" + i).src =
        LightVector[i] == 0
          ? "files/" + ThisName + "off.png"
          : "files/" + ThisName + "on.png";
      if (LightVector[i] == 1) {
  ...
  document.getElementById("B" + n).src =
    ButtonVector[n] == 0 ? "files/off.png" : "files/on.png";
}
